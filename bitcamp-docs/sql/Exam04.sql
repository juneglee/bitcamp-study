...
/* 첨부 파일 테이블 */
create table test2(
  fno int not null primary key auto_increment, /* 첨부파일 고유번호 ; 인공키*/
  filepath varchar(255) not null, /* 파일시스템에 저장된 첨부파일의 경로 */
  /* 파일의 경로만 관리하고 파일을 DBMS에 저장하는 것이 아니라 하드웨어 저장한다  */
...
- 마찬가지로 해당 게시물이 참조하는 첨부파일 데이터이기 무효한 데이터가 된다.
```
delete from test1 where no=1;
```

이런 문제가 발생한 이유>
- 다른 테이블의 데이터를 참조하는 경우, 
  참조 데이터의  존재 유무를 검사하는 제약을 걸지 않았기 때문이다 
- 테이블의 데이터를 삭제할 때 다른 테이블이 참조하는지 여부를 검사하지 않기 때문이다
해결책 
- 다른 데이터를 참조하지 않는 경우 해당 데이터의 존재 유무를 검사하도록 강제해야 한다 
  다른 데이터에 의해 참조 되는지 여부를 검사하도록 강제 해야 한다.
...
- 만약 지우고 싶다면 자식 테이블에서 삭제를 한후 사용하지 않는 데이터로만든 후에 
- 부모테이블의 데이터를 삭제 해야 한다
```
