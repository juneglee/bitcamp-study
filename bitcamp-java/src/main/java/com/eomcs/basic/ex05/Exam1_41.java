...
public class Exam1_41 {
  public static void main(String[] args) {
    ...
    // 연산 결과가 유효자릿수가 아니라면 값을 꺠진다
    //=> float과 float의 연산 결과 float이기 때문이다 
    ...
    // 출력이유?
    // float 값(988.7654...)을 
    // double 변수에 저장하기 전에 이미 float 값이 되면서 일부 값이 왜곡.
    // 그런데 r1 변수와 달리 뒤에 이상한 숫자가 많이 붙는 무엇인가?
    // IEEE 754의 이진수 변환 문제때문이다.
    // 4바이트 float 부동소수점을 8바이트 double 부동소수점 변수에 저장할 때 왜곡된 값이 들어 갈수 있다.
    // float를 double 변수에 넣을 때 왜곡이 발생하기 때문에 가능한 double 변수로 값을 바꾼 다음에 연산을 수행하라 더 좋은 것은
    // 처음부터 double 변수를 사용하라 
