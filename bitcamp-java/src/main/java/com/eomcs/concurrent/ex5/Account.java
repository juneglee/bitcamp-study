...
public class Account {
  ...
  synchronized public long withdraw(long money) {
    // long 인 경우에는 8바이트이고 기본적으로 4바이트 이기 때문에 2번 이동하며,
    // 이때 이동하는 경우, cpu가 이동하면서 잘못된 값으로 변경할 수 있다
    // 이때는 voiatile 을 이용하여 잘못되게 변경되지 않도록 한다

    // 1) 이 메서드처럼 여러 스레드가 같은 메모리(balance 필드)의 값을 동시에 변경할 때
    // 문제가 발생하는 코드를
    // "크리티컬 섹션(임계영역; critical section) 또는 크리티컬 리전(critical region)" 이라 부른다.
    ...
    for (int i = 0; i < 1000; i++)
      Math.asin(45.765); // 멈칫 멈칫하게 하여 CPU를 뺏길 기회를 제공
    // cpu를 뻇김으로써 각각의 쓰레드에게 전달될 수 있도록 한다
    b -= money;
    for (int i = 0; i < 1000; i++)
