...
public class Exam01 {
  public static void main(String[] args) {
    ...
    // 객체에 일을 시키는 방식이 엇비슷한데 
    // 메서드 시그너처가 다르기 때문에 호출할 때 일관성이 없다
    // 즉 Worker 객체를 사용하는 입장에서는 각 worker에 이떤 메서드가 있는지 확인해서 
    // 그 형식에 맞혀야 하기 때문에 객체를 사용하기가 불편한다 
    ...
    //방법
    // 객체의 사용을 정의 한다. 그리고 클래스를 정의할때 그 규칙에 따라 만든다
    // 그러면 규칙에 따라 만든 클래스를 사용할때는 일관된 방법으로 호출할 수 있어. 코딩하기가 
    // 훨씬 편하지고 유지보수가 쉬워진다
