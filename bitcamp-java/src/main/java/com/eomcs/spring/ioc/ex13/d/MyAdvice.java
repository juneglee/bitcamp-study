...
public class MyAdvice {
  ...
  /*
   * <aop:before pointcut="execution(* com.eomcs.spring.ioc.ex13.d.X.*(..)) and args(x,y)"
   * method="doBefore"/>
   */
  public void doBefore(int x, int y) {
    // 타켓 메서드의 파라미터 값을 받을 파라미터를 선언한다.
    // AOP 설정할 때 파라미터 값을 받을 변수 이름을 지정한다.
    // 위 규칙에 따르면,
    // 타켓 메서드의 첫 번째 파라미터 값은 y 변수에서 받을 것이고
  ...
  /*
   * <aop:after-returning pointcut="execution(* com.eomcs.spring.ioc.ex13.d.X.*(..))"
   * method="doAfterReturning" returning="returnValue"/>
   */
  public void doAfterReturning(Object returnValue) {
    // 타켓 메서드의 리턴 값은 설정 파일에 정의한 대로
