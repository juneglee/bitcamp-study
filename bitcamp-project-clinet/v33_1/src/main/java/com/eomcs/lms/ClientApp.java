...
public class ClientApp {
  ...
  // 클래스에서 필드를 선언하고 생성자에서 객체를 준비하지만,
  // 이곳에 Queue<String> commandQueue = new LinkedList<>(); 를 만들어도 된다
  ...
  private void processCommand(String command) {
    try (Socket socket = new Socket(host, port); // 명령이 들어오면 서버에 접속한다
      ...
      commandMap.put("/server/stop", () -> {
        try {
          out.writeUTF(command); // server/stop이 명령어를 받으면 서버에 보낸다
          out.flush();
      ...
      Command commandHandler = commandMap.get(command); // 명령어를 처리할 객체를 찾는다
